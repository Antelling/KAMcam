...
export class ElasticSystem implements System {
  ...
  build(root: TgpuRoot, config: SimulationConfig, cellCount: number): void {
    ...
    this.StateArrayA = d.arrayOf(ElasticStateA, cellCount);
    this.StateArrayB = d.arrayOf(ElasticStateB, cellCount);
    this.DataArray = d.arrayOf(DataCell, cellCount);
    this.paramsBuffer = root.createBuffer(ElasticParams, this.buildParamsData(config)).$usage('uniform');
    ...
    const initCell = (cellIndex: number) => {
      ...
      const omu = 1.0 - u;
      const omv = 1.0 - v;
      initLayout.$.stateA[cellIndex] = ElasticStateA({
      ...
      initLayout.$.stateB[cellIndex] = ElasticStateB({
    ...
    const stepCell = (cellIndex: number) => {
      ...
      stepLayout.$.nextB[cellIndex] = ElasticStateB({
